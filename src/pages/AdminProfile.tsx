...
import { User, Mail, Phone, ShieldCheck, Loader2, Save, Briefcase, KeyRound } from "lucide-react";
...
export const AdminProfile: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/v1/admin/${user?.id || user?._id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
        body: JSON.stringify({ fullName, email, phonenumber, department }),
      });
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 text-left min-h-[85vh]">
      <div className="border-b border-border-light pb-6 mb-8">
        ...
        <p className="text-xs text-text-light font-sans font-normal mt-1 uppercase tracking-wider">
          Manage your departmental contact credentials and administrative identifiers.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="bg-white border border-border-light rounded-3xl p-6 flex flex-col items-center text-center shadow-[0_12px_30px_rgba(45,106,79,0.03)]">
          ...
          <span className="text-[10px] font-sans font-bold text-[#1b4332] bg-[#e8f5e9] px-3 py-1 mt-2.5 rounded-full uppercase tracking-wider block">
            Admin: {user?.department || "MUNICIPAL OFFICE"}
          </span>

          <div className="mt-6 border-t border-border-light pt-5 w-full flex items-center justify-center gap-1.5 text-[10px] font-sans font-bold text-text-light uppercase tracking-wider">
            <KeyRound className="h-4 w-4" />
            <span>Access Code: <strong className="text-ink font-bold">{user?.adminAccessCode}</strong></span>
          </div>
        </div>
        ...
        <div className="md:col-span-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                <Briefcase className="absolute top-3.5 left-4 h-4 w-4 text-text-light" />
                <input
                  required
                  type="text"
                  value={department}
